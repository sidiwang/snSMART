#' Trial dataset (dose level design)
#'
#' Generate trial dataset for dose level design without missing value
#'
...
#' BJSM binary (dose level design)
#'
#' BJSM (Bayesian Joint Stage Modeling) method that borrows information across both stages to estimate the individual response rate of each dose level (with binary outcome) based on trial dataset
...
#' @param pi_prior.a parameter a of the prior distribution for pi (response rate) of placebo. Please check the `Details` section for more explaination
#' @param pi_prior.b parameter b of the prior distribution for pi (response rate) of placebo. Please check the `Details` section for more explaination
#' @param beta_prior.a parameter a of the prior distribution for linkage parameter beta
#' @param beta_prior.b parameter b of the prior distribution  for linkage parameter beta
#' @param n_MCMC_chain number of MCMC chains, default to 1. If this is set to a number more than 1
#' @param normal.mean our function assumes that the logarithm of treatment effect ratio follows a Gaussian prior distribution `N(\mu, \sigma^2)`, that is `log(\pi_L/\pi_P)~N(normal.mean, normal.var`, and `log(\pi_H/\pi_P)~N(normal.mean, normal.var`. `normal.mean` is the mean of this Gaussian prior
