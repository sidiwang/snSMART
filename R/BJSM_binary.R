...
#'
#' @return a `matrix` of the trial dataset input by user
...
#' beta0_prior_dist = "beta", beta0_prior_a = 1.6, beta0_prior.b = 0.4, beta1_prior_dist = "pareto", beta1_prior.a = 3, beta1_prior_c = 1,
  ...
  if (six == TRUE){
    ...
    if (DTR == TRUE){
      ...
      result = list("posterior_sample" = out_post, # posterior samples of the link parameters and response rates generated through the MCMC process
                    ...
                    "diff_BC" = apply(out_post[,8] - outpost[,9],2,mean), # estimate of differences between treatments B and C
                    "diff_AC" = apply(out_post[,7] - outpost[,9],2,mean), # estimate of differences between treatments A and C
                    "ci_diff_AB" = bayestestR::ci(out_post[,7] - outpost[,8], ci = ci, method = "HDI"), # x% credible intervals for the differences between A and B
                    "ci_diff_BC" = bayestestR::ci(out_post[,8] - outpost[,9], ci = ci, method = "HDI"), # x% credible intervals for the differences between B and C
                    "ci_diff_AC" = bayestestR::ci(out_post[,7] - outpost[,9], ci = ci, method = "HDI"), # x% credible intervals for the differences between A and C
    ...
    if (DTR == TRUE){
      result = list("posterior_sample" = out_post, # posterior samples of the link parameters and response rates generated through the MCMC process
                    ...
                    "diff_BC" = apply(out_post[,4] - outpost[,5],2,mean), # estimate of differences between treatments B and C
                    "diff_AC" = apply(out_post[,3] - outpost[,5],2,mean), # estimate of differences between treatments A and C
                    "ci_diff_AB" = bayestestR::ci(out_post[,3] - outpost[,4], ci = ci, method = "HDI"), # x% credible intervals for the differences between A and B
                    "ci_diff_BC" = bayestestR::ci(out_post[,4] - outpost[,5], ci = ci, method = "HDI"), # x% credible intervals for the differences between B and C
                    "ci_diff_AC" = bayestestR::ci(out_post[,3] - outpost[,5], ci = ci, method = "HDI"), # x% credible intervals for the differences between A and C
