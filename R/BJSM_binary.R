...
#' @param pi_prior for 3 active treatment design: vector of six values (a, b, c, d, e, f), where a and b are the parameter \code{a} and parameter \code{b} of the prior distribution for \code{pi_1A}, c and d are the parameter \code{a} and parameter \code{b} of the prior distribution for \code{pi_1B}, and e and f are the parameter \code{a} and parameter \code{b} of the prior distribution for \code{pi_1B}. for dose level design: vector of two values (a, b). `a` is the parameter `a` of the prior distribution for \code{pi} (response rate) of placebo. `b` is the parameter `b` of the prior distribution for \code{pi} of placebo. Please check the `Details` section for more explanation
#' @param beta_prior for 3 active treatment design: vector of four values (a, b, c, d). `a` is the value of parameter \code{a} of the prior distribution for linkage parameter \code{beta0}, `b` is the value of parameter \code{b} of the prior distribution for linkage parameter \code{beta0}. `c` is the value of parameter \code{a} of the prior distribution for linkage parameter \code{beta1}. `d` is the value of parameter \code{b} of the prior distribution for linkage parameter \code{beta1}. for dose level design: vector of two values (a, b). `a` is the parameter `a` of the prior distribution for linkage parameter \code{beta}. `b` is the parameter b of the prior distribution  for linkage parameter \code{beta}. Please check the `Details` section for more explanation
